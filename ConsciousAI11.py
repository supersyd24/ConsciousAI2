...
# Schema memory (Cerebral Cortex)
cerebral_cortex = []  # {"input": str, "embedding": tensor, "entropy": float, "name": str, "path": str}
...
def show_schemas():
    print("\n--- Schema Memory ---")
    for idx, schema in enumerate(cerebral_cortex):
...
def main():
    ...
    while True:
        ...
        if exact_match and matched_schema:
            ...
            for param in model.parameters():
                param.requires_grad = True  # ✅ Ensure gradients can flow
    ...
    print("\nConversation Ended.")
    print(f"Total schemas saved: {len(cerebral_cortex)}")
